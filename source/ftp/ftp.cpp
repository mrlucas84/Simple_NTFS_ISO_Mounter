...
const char* VERSION = "for FB Alpha RL";	// used in the welcome message and displayed on-screen
	...
	if(list_s > 0)
	{
		while(exitapp == 0 /*&& !mm_shutdown*/)
		{
			...
			if((conn_s = accept(list_s, NULL, NULL)) > 0)
			{
				ftp_clients++;
				sys_ppu_thread_t id;
				sys_ppu_thread_create(&id, handleclient, (u64)conn_s, 0, 0x1000 + BUFFER_SIZE, 0, "ClientCmdHandler");
			}
		}
...
int main_ftp(u8 _mode)
{
	...
	sys_ppu_thread_t id;
	sys_ppu_thread_create(&id, handleconnections, 0, 1500, 0x100000, 0, "ServerConnectionHandler");
